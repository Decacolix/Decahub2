...
const formatTemperatureWithUnit = (
	temperature: number,
	temperatureUnit: TemperatureUnit,
...
const CurrentWeatherTile = ({
	...
	weather,
	windSpeedUnit,
}: CurrentWeatherTileProps) => {
	const weatherVisual = getWeatherVisual(weather.weatherCode);

	return (
		<article className="text-white mb-8">
			<header className="pr-10">
				<h2 className="mt-1 text-3xl font-semibold [text-shadow:0_2px_8px_rgba(0,0,0,0.45)]">{locationName}</h2>
			</header>

			<div className="mt-2 flex flex-row items-start justify-between gap-3 lg:gap-4">
				<div>
			...
			<div className="mt-5 flex flex-row justify-between">
				...
				<div className="flex items-center gap-3">
					...
					<p>
						...
						<strong className="text-xl font-semibold tabular-nums [text-shadow:0_2px_8px_rgba(0,0,0,0.45)]">{numberFormatter.format(weather.windSpeed)} {windSpeedUnitLabels[windSpeedUnit]}</strong>
					</p>
				</div>
			</div>
		</article>
