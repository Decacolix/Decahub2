...
const WeatherPanel = ({ isSettingsOpen, location, temperatureUnit, windSpeedUnit, onToggleSettings }: WeatherPanelProps) => {
	const { data, status } = useWeather(location, temperatureUnit, windSpeedUnit);

	return (
		<aside aria-label={`Počasí pro ${location.name}`} className="group font-outfit relative w-full min-w-0">
			<button
			...
			{data ? (
				<>
					<CurrentWeatherTile locationName={location.name} temperatureUnit={data.temperatureUnit} weather={data.current} windSpeedUnit={data.windSpeedUnit} />
				...
				<p className="p-6 text-center text-lg text-white [text-shadow:0_2px_8px_rgba(0,0,0,0.45)]">{status === 'error' ? 'Počasí se nepodařilo načíst.' : 'Načítání počasí…'}</p>
			)}
		</aside>
	);
};
